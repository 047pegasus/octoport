...
(() => {
  ...
  function loadApexCharts() {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.src = "/assets/apexcharts.min.js";
      script.onload = () => {
  ...
  function initChart(container) {
    ...
    console.log(`[octoport] initChart: id="${containerId}", name="${chartName}", data-points:`, chartValues ? JSON.parse(chartValues).flat().length : 0);
    ...
    const chartBuilders = {
      radar: () => buildRadarChartOptions(CHART_DATA, CHART_LABELS, colors, container, currentTheme),
      ...
      pie: () => buildPieChartOptions(CHART_DATA, CHART_LABELS, colors, container, currentTheme),
      radial: () => buildRadialChartOptions(CHART_DATA, CHART_LABELS, colors, container, currentTheme),
    };

    const options = chartBuilders[chartType]?.();

    if (options && typeof ApexCharts !== "undefined") {
      console.log(`[octoport] Creating ApexCharts instance for ${containerId}`);
      const chart = new ApexCharts(container, options);
      chart.render().then(() => {
        console.log(`[octoport] Chart rendered successfully: ${containerId}`);
        ...
        console.error(`[octoport] Chart render failed for ${containerId}:`, err);
      });
  ...
  async function checkAndInitCharts() {
    ...
    console.log(`[octoport] checkAndInitCharts: found ${containers.length} chart containers, ApexCharts:`, typeof ApexCharts);
    ...
    if (containers.length > 0 && typeof ApexCharts === "undefined") {
      try {
        ...
        console.log("[octoport] ApexCharts loaded successfully");
      } catch (e) {
        console.error("[octoport] Failed to load ApexCharts:", e);
        return;
      }
    }
    ...
    containers.forEach((container) => {
      if (container.dataset.name && typeof ApexCharts !== "undefined") {
        ...
        if (needsInit) {
          console.log(`[octoport] Initializing chart: ${containerId}`);
          initChart(container);
        } else {
          console.log(`[octoport] Skipping init for ${containerId} (already initialized)`);
        }
      }
    });
  }
  ...
  // Diagnostics: snapshot the engine state so the host app can log why a chart
  // is (or isn't) rendering.
  window.__octoportCharts = () => JSON.stringify({
    apex: typeof ApexCharts,
    ready: document.readyState,
    divs: Array.from(document.querySelectorAll("div[data-chart-values]")).map((d) => ({
      id: d.id,
      name: d.dataset.name,
      init: d.dataset.chartInitialized === "true",
    })),
    instances: Array.from(chartInstances.keys()),
    theme: document.documentElement.dataset.theme,
  });

  // Imperative realtime updates: push fresh series + labels into an already-
  // rendered chart without destroying/recreating it (used by the OctoPort GUI's
  // 60fps animation loop so curves glide instead of snapping on each tick).
  window.__octoportUpdateChart = (id, valuesJson, labelsJson, namesJson) => {
    const chart = chartInstances.get(id);
    if (!chart) {
      console.warn(`[octoport] __octoportUpdateChart: chart not found for id="${id}", available:`, Array.from(chartInstances.keys()));
      return false;
    }
    let values, labels, names;
    try { values = JSON.parse(valuesJson || "[]"); } catch { return false; }
    try { labels = JSON.parse(labelsJson || "[]"); } catch { labels = []; }
    try { names = JSON.parse(namesJson || "[]"); } catch { names = []; }
    const el = document.getElementById(id);
    const name = (el && el.dataset.name) || "";
    const isMulti = Array.isArray(values[0]);
    if (name.includes("Pie") || name.includes("Radial")) {
      chart.updateOptions({ series: values, labels }, false, true);
    } else if (name.includes("Radar")) {
      chart.updateOptions({ series: [{ name: names?.[0] || "Metrics", data: values }], xaxis: { categories: labels } }, false, true);
    } else if (isMulti) {
      const series = values.map((d, i) => ({ name: names?.[i] || `Series ${i + 1}`, data: d }));
      chart.updateOptions({ series, xaxis: { categories: labels } }, false, true);
    } else {
      chart.updateOptions({ series: [{ name: names?.[0] || "Value", data: values }], xaxis: { categories: labels } }, false, true);
    }
    return true;
  };

  // Wait for a chart to be initialized. Returns a promise that resolves when
  // the chart instance exists in chartInstances.
    ...
    return new Promise((resolve, reject) => {
      ...
      const check = () => {
        ...
        if (Date.now() - start > timeoutMs) {
          reject(new Error(`Timeout waiting for chart ${id}`));
          return;
        }
        setTimeout(check, 50);
      };
  ...
  function startObserver() {
    ...
    observer.observe(document.documentElement, {
      attributes: true,
      attributeFilter: ["data-color-theme", "style", "class", "data-theme"],
    });
    checkAndInitCharts();
  }
  ...
  // Also run an immediate check in case containers are already in DOM
  // (common in SPA frameworks where components render before scripts load)
  setTimeout(checkAndInitCharts, 0);
...
function buildRadarChartOptions(data, labels, colors, container, theme) {
...
function buildLineChartOptions(data, labels, colors, container, theme) {
  ...
  // Professional smooth curves: use low tension (0.25) for elegant curves
  // without wobble. The Rust side does the heavy lifting at 60fps.
  const tension = container.dataset.chartTension ? parseFloat(container.dataset.chartTension) : 0.25;
...
function buildPieChartOptions(data, labels, colors, container, theme) {
  const styles = getComputedStyle(document.documentElement);
  const containerHeight = container.offsetHeight || 400;
  return {
    series: data,
    chart: { type: "pie", height: containerHeight, animations: { enabled: true, speed: 800, animateGradually: { enabled: true, delay: 150 } } },
    labels,
...
function buildRadialChartOptions(data, labels, colors, container, theme) {
  const styles = getComputedStyle(document.documentElement);
  const containerHeight = container.offsetHeight || 400;
  return {
    series: data,
    chart: { type: "radialBar", height: containerHeight, animations: { enabled: true, speed: 800, animateGradually: { enabled: true, delay: 150 } } },
    plotOptions: {
      radialBar: {
...
function buildAreaChartOptions(data, labels, colors, container, theme) {
  ...
  const animationsEnabled = container.dataset.chartAnimations !== "false";

  // Professional smooth curves: use "straight" for raw data, but for our
  // pre-smoothed 60fps data, "smooth" with low tension (0.25) gives
  // elegant curves without wobble. The Rust side does the heavy lifting.
  const tension = container.dataset.chartTension ? parseFloat(container.dataset.chartTension) : 0.25;

  // Build stroke config with tension for professional smooth curves
  const strokeConfig = curveType === "smooth" 
    ? { curve: "smooth", width: 2, tension: tension }
    : { curve: curveType, width: 2 };

  return {
    series,
    chart: { type: "area", height: containerHeight, toolbar: { show: false }, animations: animationsEnabled ? { enabled: true, speed: 800, animateGradually: { enabled: true, delay: 150 } } : { enabled: false }, stacked, stackType: stackType === "100%" ? "100%" : undefined },
    plotOptions: { area: { stacked: stacked || undefined } },
